...
function VerifyOtp() {
  ...
  const handleVerify = async (e) => {
    ...
    try {
      ...
      setMessage({ 
        type: "error", 
        text: error.response?.data?.message || "Invalid security code. Please check and try again." 
      });
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="min-h-screen w-screen flex items-center justify-center bg-[#f3f2f1] font-sans antialiased text-[#323130] p-4 select-none">
      ...
      <div className="w-full max-w-[440px] bg-white p-11 rounded-sm border border-[#d2d0ce]" style={{ boxShadow: "0 2px 12px rgba(0,0,0,0.06)" }}>
        ...
        <div className="mb-6">
          ...
          <p className="text-xs mt-2 leading-relaxed" style={{ color: "#605e5c" }}>
            We sent a verification code to your security identifier. Please insert the code sequence below.
          </p>
          {email && (
            <p className="text-xs font-semibold text-[#0078d4] mt-2 bg-[#eff6fc] px-2.5 py-1 rounded-sm border border-[#deecf9] truncate w-fit max-w-full font-mono tracking-wide">
              {email}
            </p>
          )}
        </div>
        ...
        <form onSubmit={handleVerify} className="space-y-6">
          ...
          {/* Underlined Microsoft Style Input */}
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <button
              ...
              className="min-w-[100px] text-white text-xs font-semibold px-5 py-2.5 bg-[#0078d4] hover:bg-[#106ebe] active:bg-[#005a9e] disabled:bg-[#f3f2f1] disabled:text-[#a19f9d] disabled:border-[#edebe9] transition-all flex items-center justify-center gap-1 shadow-xs cursor-pointer rounded-none"
